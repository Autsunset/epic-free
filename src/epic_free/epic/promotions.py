...
async def get_promotions() -> List[PromotionGame]:
    ...
    try:
        ...
        # Epic's parameter is ``locale`` (the previous ``local`` was a typo and
        # was silently ignored, so the storefront fell back to its default).
        resp = await client.get(URL_PROMOTIONS, params={"locale": "zh-CN"})
        data = resp.json()
